...
async def _handle_local_process_and_respond(message: types.Message, input_path: Path):
    try:
        ...
        if size <= MAX_TELEGRAM_FILESIZE:
            ...
            url = await loop.run_in_executor(None, upload_to_pixeldrain, str(output_path), output_path.name)
            if url:
...
async def handle_incoming_media(message: types.Message):
    if message.content_type not in [ContentType.DOCUMENT, ContentType.VIDEO, ContentType.PHOTO]:
        return
